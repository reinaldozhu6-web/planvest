...
import mobileNavigation from "./mobileNavigation.json";
...
function MobileNav({ onClose, onLogout }: { onClose: () => void; onLogout: () => void }) {
  return <><button type="button" className="mobile-nav-backdrop" aria-label="Close navigation" onClick={onClose} /><aside id="mobile-navigation" className="mobile-nav-drawer" aria-label="Mobile navigation"><Link className="brand" href="/" onClick={onClose}><span>PV</span>PlanVest</Link><nav>{mobileNavigation.items.map(item => <a key={item.href} href={item.href} onClick={onClose}>{item.label}</a>)}</nav><button type="button" className="mobile-nav-logout" onClick={onLogout}><LogOut />{mobileNavigation.logoutLabel}</button></aside></>;
}
...
export default function Dashboard() {
  ...
  return <main className="dashboard"><SideNav name={data.user.displayName} onLogout={() => void signOut()} />{mobileNavOpen && <MobileNav onClose={() => setMobileNavOpen(false)} onLogout={() => void signOut()} />}<section className="dashboard-main" id="overview"><header className="dash-topbar"><button type="button" className="mobile-menu" aria-label={mobileNavOpen ? "Close navigation" : "Open navigation"} aria-expanded={mobileNavOpen} aria-controls="mobile-navigation" onClick={() => setMobileNavOpen(value => !value)}><Menu /></button><div><p>{new Date().toLocaleDateString("en-CA", { weekday: "long", month: "long", day: "numeric" })}</p><h1>Welcome back, {firstName}.</h1></div><div className="top-actions"><button aria-label="Notifications"><Bell /></button><span className="workspace-label">CAD workspace</span></div></header><div className="demo-banner"><ShieldCheck /><span><b>Educational planning workspace</b> — Prices are manually entered and all projections are estimates, not financial advice.</span></div>{error && <p className="workspace-error" role="alert">{error}</p>}
